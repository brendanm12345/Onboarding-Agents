...
def perform_query_and_download(page: Page) -> None:
    """
    Submits the query form and waits for the query to complete.
    Handles rerun if initial query faces an error.
    """
    expect(page).to_have_url(
        "https://wrds-www.wharton.upenn.edu/pages/get-data/center-research-security-prices-crsp/annual-update/stock-security-files/monthly-stock-file/"
    )
    ...
    expect(queried_page).to_have_url(re.compile(r".*/query-manager/query/.*"))

    rerun_if_needed(queried_page)


def rerun_if_needed(page: Page) -> None:
    """
    Checks for rerun option and performs rerun if present.
    """
    try:
        page.get_by_role("link", name="Rerun").click()
        page.get_by_role("button", name="Submit Form").click()
        expect(page).to_have_url(re.compile(r".*/query-manager/query/.*"))
    except Exception:
        # Log or handle as needed - assume rerun not needed
        pass
